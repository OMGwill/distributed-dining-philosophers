...
public class Diner implements IDinerNeighbor {
	...
	throws RemoteException {
		...
		showState();
		if (myState == DinerState.THINKING && rightFork.isMine() && !rightFork.isClean())
		{
			rightFork.setClean();
			sendMyRightFork();
		}
	...
	throws RemoteException {
		...
		showState();
		if (myState == DinerState.THINKING && leftFork.isMine() && !leftFork.isClean())
		{
			leftFork.setClean();
			sendMyLeftFork();
		}
	...
	protected void showState() {
		try {
			...
			if (leftFork.isMine()) {
				...
				if(leftFork.isClean())
					System.out.print(" Left Fork is clean");
			    if(!leftFork.isClean())
					System.out.print(" Left Fork is dirty");
			}
			...
			if (rightFork.isMine()) {
				...
				if(rightFork.isClean())
					System.out.print(" Right Fork is clean");
				if(!rightFork.isClean())
					System.out.print(" Right Fork is dirty");
	...
	throws RemoteException {
		...
		if (myState == DinerState.EATING)
			return;
		if (myState != DinerState.EATING && rightFork.isReleasable() && !rightFork.isClean())
		{
			rightFork.setClean();
			sendMyRightFork();
		}
		if (myState != DinerState.EATING && leftFork.isReleasable() && !leftFork.isClean())
		{
			leftFork.setClean();
			sendMyLeftFork();
		}
		//if (myState == DinerState.HUNGRY && leftFork.isReleasable() && !leftFork.isClean() && !rightFork.isMine())
		//{
		//	leftFork.setClean();
		//	sendMyLeftFork();
		//}
	...
	throws RemoteException {
		...
		if (rightFork.isMine() && leftFork.isMine() && rightFork.isClean() && leftFork.isClean())
